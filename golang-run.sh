...
function portForwarding() {
  ...
  elif [[ -d "/go/src/backend/common-library/module/" ]]; then
    echo "pure folder is found."
    input_files=(
      "/go/src/backend/common-library/module/.pure-env"
      "/go/src/backend/common-library/module/local-grpc.json"
    )
  ...
  for ((i = 0; i < ${#input_files[@]}; i++)); do
    ...
    # 讀取檔案的每一行
    while IFS= read -r line || [[ -n "$line" ]]; do # 或是讀取到最後一行, 沒有換行字元
      # 解析 key 和 value
      key="${line%%=*}"
      value="${line##*=}"
      ...
      if [[ "$key" == GRPC_* || "$key" == grpc.* ]]; then
        ...
        service_key=$(echo "$key" |
          sed 's/CLOUDMICRO/CLOUD_MICRO_/g' | tr '[:upper:]' '[:lower:]' | sed 's/grpc.micro/grpc.micro./g' |
          sed 's/_/./g' | sed 's/micro.//g' | sed 's/grpc.//g' | sed 's/\./-/g')
